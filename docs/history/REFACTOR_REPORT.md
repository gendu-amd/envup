...
> 备注（后续补记）：本节当时的对比依据 `.understand-anything/knowledge-graph.json` 是一份**冻结的重构前快照**，原计划事后刷新，实际没有——它一路停留在重构前的代码布局上，直到 v0.2 收尾时才被发现已落后 28 个提交、90 → 168 个文件。图谱按 commit 定格却又提交进仓库，只要有人合入改动它就是错的，而且没有任何机制会指出来。该目录已整体移出版本管理（见 `.gitignore`），需要时在本地重新生成。
