...
lib/
  ...
  clangd.sh            # translate Docker CMake databases into host-view cache
...
`envup clangd sync` is deliberately a CLI workflow rather than an install
module: it operates on one project's generated build metadata. It identifies
the project and Docker source mount, applies every inspected mount mapping to a
copy of `compile_commands.json`, and registers that copy under Nvim's cache.
The original build tree remains Docker-owned; Nvim selects the cached database
per LSP root and warns when the source database changes.
...
| `GH_REPO` / `GH_TAG` / `GH_BINS` / `SCRIPT_URL` / `GIT_URL` / … | provider parameters; `GH_TAG` overrides the lock when compatibility requires it, and `GH_BINS` installs companion commands |
...
- **Backup, never clobber.** `safe_link` moves any pre-existing file, directory or
  foreign symlink at a link
...
Runtime state follows the same rule. When `$HOME` is on a shared filesystem,
`$ENVUP_STATE_DIR` is automatically scoped to `hosts/<short-hostname>` so the
manifest, created-file ledger, adopt stash and logs from two machines cannot race
or overwrite one another. An explicit `ENVUP_STATE_DIR` opts out. A legacy
unscoped manifest remains readable and is copied, not moved, on the first write
from each host.
