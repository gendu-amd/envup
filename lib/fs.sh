...
_link() {
    ...
    # Anything already occupying the destination belongs to the user unless it
    # is the exact link requested above. Preserve regular files, directories,
    # foreign symlinks and dangling symlinks alike; replacing a symlink without
    # a backup is just as destructive as replacing a file.
    if [[ -e "$dst" || -L "$dst" ]]; then
