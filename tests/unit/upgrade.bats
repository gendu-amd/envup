#!/usr/bin/env bats
# Upgrade diagnostics against a real local origin and clone.
...
mk_repo() {
    ...
    # envup follows entry-point symlinks (the production ~/.local/bin layout),
    # so this fake checkout needs its own copy to remain the CLI's root.
