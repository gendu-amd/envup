#!/usr/bin/env bats
# Static and sandboxed interactive checks for the zsh configuration.
...
zrun() {
    ...
    env -i HOME="$ZHOME" ZDOTDIR="$ZHOME" TERM=xterm \
        PATH="$STUB_BIN:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
        zsh -i -c "$1"
}
...
@test "compinit is called at most once outside the no-OMZ fallback (B9)" {
    ...
    grep -q 'compinit -i -d' "$ZDIR/40-shell.zsh"
}
...
@test "extra completions join fpath before the single compinit" {
    local f="$ZDIR/40-shell.zsh"
    grep -q 'zsh-completions/src' "$f"
    local extra omz
    extra="$(code "$f" | grep -n '_zsh_completions=' | cut -d: -f1)"
    omz="$(code "$f" | grep -n 'oh-my-zsh.sh' | head -1 | cut -d: -f1)"
    [ -n "$extra" ] && [ -n "$omz" ] && [ "$extra" -lt "$omz" ]
}

@test "fzf loads before fzf-tab and widget wrappers" {
    local f="$ZDIR/40-shell.zsh" fzf auto
    grep -q 'commands\[fzf\].*plugins+=(fzf fzf-tab)' "$f"
    fzf="$(code "$f" | grep -n 'plugins+=(fzf fzf-tab)' | cut -d: -f1)"
    auto="$(code "$f" | grep -n 'plugins+=(zsh-autosuggestions' | cut -d: -f1)"
    [ -n "$fzf" ] && [ -n "$auto" ] && [ "$fzf" -lt "$auto" ]
}

@test "the tools slice does not overwrite fzf-tab's final Tab binding" {
    local f="$ZDIR/50-tools.zsh"
    grep -q 'widgets\[fzf-file-widget\]' "$f"
    grep -q 'widgets\[fzf-tab-complete\]' "$f"
    local guard source_line
    guard="$(code "$f" | grep -n 'widgets\[fzf-file-widget\]' | cut -d: -f1)"
    source_line="$(code "$f" | grep -n 'source ~/.fzf.zsh' | cut -d: -f1)"
    [ -n "$guard" ] && [ -n "$source_line" ] && [ "$guard" -lt "$source_line" ]
}
...
@test "a fresh interactive shell prints nothing to stderr" {
    command -v zsh >/dev/null || skip "zsh is not installed"
    zsh_home
    # -d disables distro-wide global rc files. Ubuntu's /etc/zshrc runs
    # compinit and complains when CI has no tty; this test owns ZDOTDIR and is
    # specifically checking envup's startup output, not the runner image.
    ...
    [ -z "$output" ] || {
        printf 'unexpected zsh startup output: %q\n' "$output" >&3
        return 1
    }
}
...
@test "yazi changes directory through a guarded shell wrapper" {
    local f="$ZDIR/65-func.zsh"
    grep -q 'commands\[yazi\]' "$f"
    grep -q -- '--cwd-file=' "$f"
    grep -q 'builtin cd --' "$f"
}

@test "the yazi wrapper returns the parent shell to the chosen directory" {
    stub_bin yazi <<'EOF'
#!/bin/sh
for arg in "$@"; do
    case "$arg" in --cwd-file=*) printf '%s' "$HOME/target" > "${arg#*=}" ;; esac
done
EOF
    run zrun 'mkdir -p "$HOME/target"; y; print "PWD=$PWD"'
    [ "$status" -eq 0 ]
    [[ "$output" == *"PWD=$TEST_TMP/zhome/target"* ]] || { echo "$output"; false; }
}
