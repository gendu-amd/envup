#!/usr/bin/env bats
# OSC 52 and native clipboard behavior across tmux and Nvim.
...
@test "the tmux clipboard config is valid tmux syntax" {
    ...
    # Some restricted containers expose the binary but forbid the Unix socket
    # tmux needs. Prove the runtime works before attributing that failure to the
    # config. A real session also keeps tmux 2.x alive; start-server alone exits
    # immediately on those versions.
    run tmux -L "$sock-preflight" -f /dev/null new-session -d
    if [ "$status" -ne 0 ]; then
        skip "cannot start a tmux server here"
    fi
    tmux -L "$sock-preflight" kill-server 2>/dev/null || true

    run tmux -L "$sock" -f "$(TMUX_CONF)" new-session -d \; \
...
@test "the terminal-side setup is documented, because envup cannot do it" {
    ...
    grep -q 'enableClipboardWrite' "$REPO_ROOT/README.md"
    grep -q 'enableClipboardWrite' "$REPO_ROOT/README.zh-CN.md"
}
