...
# Leave the shell in Yazi's final directory. The binary remains useful without
# zsh; this wrapper is only the small piece a child process cannot do itself.
if (( $+commands[yazi] && ! $+commands[y] && ! $+functions[y] )); then
    y() {
        local cwd_file cwd rc
        cwd_file="$(mktemp "${TMPDIR:-/tmp}/envup-yazi.XXXXXX")" || return 1
