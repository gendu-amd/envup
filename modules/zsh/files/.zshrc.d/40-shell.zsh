...
# Extra completion definitions must join fpath before Oh-My-Zsh's one compinit.
_zsh_completions="${ZSH_CUSTOM:-$ZSH/custom}/plugins/zsh-completions/src"
...
# Load fzf's bindings before fzf-tab replaces Tab. Loading fzf again later
# would silently restore its own completion widget over fzf-tab.
...
if [[ -f "${ZSH}/oh-my-zsh.sh" ]]; then
    ...
    # The shell still has to work, so do the minimum ourselves — once. Ignore
    # insecure completion directories instead of trusting them (-u) or trying
    # to prompt on an SSH/CI process that may not have a terminal.
