...
#   Tab      fuzzy normal completion  — fzf-tab (when available)
...
# Oh-My-Zsh loads fzf before fzf-tab. The fallback keeps Ctrl-T/Alt-C and
# **<Tab> available when Oh-My-Zsh was unavailable during installation.
if (( ! $+widgets[fzf-file-widget] && ! $+widgets[fzf-tab-complete] )) &&
        [[ -f ~/.fzf.zsh ]]; then
    source ~/.fzf.zsh
fi
