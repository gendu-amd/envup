...
GH_TAG="v1.2.3"                # optional release-tag override (default: versions.lock)
GH_BINS=(tool helper)           # optional: install several binaries from one archive
...
- Keep user-facing usage concise and add it to the README. Add a separate guide
  only when the README cannot explain the feature clearly.
...
- **Unit tests** (`tests/unit/`) cover the library, one file per concern:
  ...
  bare binary with no archive around it), `productivity` (jq/tldr/TUI modules
  and the profile boundary that keeps TUIs optional), `checksum` (digest computation
...
Steps 1–3 have to agree with each other. `tests/unit/release.bats` checks all
three: `VERSION` against the newest
