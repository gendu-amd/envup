...
# Install the standard profile (shell, tmux, search tools, jq and tldr)
./envup install
...
[i] install order: zsh git tmux fzf ripgrep fd bat eza zoxide atuin delta direnv jq tealdeer
==> [zsh] install
✓ linked: ~/.zshrc
...
==> [zoxide] install
...
✓ ok:       zsh git fzf ripgrep fd bat eza zoxide atuin delta direnv jq tealdeer
...
Modules:
  ...
  ○ nvim     Neovim + NvChad with pinned plugins and language tools
...
./envup clangd sync [--container NAME] [PATH]                                # Translate a Docker CDB for host Nvim
./envup clangd status|clean [PATH]                                           # Inspect/remove that project cache
...
- `status` reports what is **actually true on disk right now** — it re-reads every symlink and re-runs every version check. `✓ ok` / `~ degraded` / `! needs attention` / `○ not installed`. Delete a config by hand and status says `!` on the next run.
- A `!` names which of three things happened, because the fixes differ: **`N links not created yet`** (the repo grew a link since you last installed — run `envup install <module>`, nothing is damaged), **`N dangling links`** (the source or the checkout moved — `envup doctor --fix`), **`N paths already in use`** (your own file sits where a link goes; envup reports it rather than moving your file).
- `status --json` prints the same thing machine-readably (`state`, `tool`, `provider`, `version`, and `link_issues` as `{unlinked, dangling, foreign}` per module, plus platform / package manager / privilege level).
...
| `standard` (default) | `+ tmux fzf ripgrep fd bat eza zoxide atuin delta direnv jq tealdeer` | Typical developer workstation |
| `full` | `+ nvim lazygit yazi` | Power-user workstation |
...
# profiles/full.sh = standard + editor and terminal TUIs
...
| `jq` | JSON processor for APIs, logs and pipelines | — |
| `tealdeer` | Fast practical command examples through `tldr` | — |
| `nvim` | Neovim with NvChad (plugins pinned via lazy-lock.json) | — |
| `lazygit` | Terminal UI for staging, history, branches and rebases | `git` |
| `yazi` | Vim-style terminal file manager; `y` keeps its final directory | — |

With the full profile, use `lg` for lazygit and `y` for Yazi; normal Tab
completion becomes fuzzy when `fzf` is present. `tldr <command>` and `jq` are
available in the default profile.

### Everyday use

After the first install, start a new shell with `exec zsh`. After pulling a
version that adds modules, include the profile so those new modules are picked
up too:

```bash
./envup upgrade --profile standard   # or: full
exec zsh
```

| What you want | Command / key |
|---------------|---------------|
| Switch zsh between insert and normal mode | `Esc` to normal mode; `i` / `a` back to insert mode |
| Complete a command, option or path fuzzily | `Tab` (`fzf-tab`, when fzf is installed) |
| Search command history | `Ctrl-R` (Atuin; fzf is the fallback) |
| Insert a selected file / change to a selected directory | `Ctrl-T` / `Alt-C` |
| Jump to a known directory / choose one interactively | `z <name>` / `zi` |
| Return to restored tmux work / choose a project session | `tm` / `ts` |
| Move across tmux panes and nvim splits | `Ctrl-h/j/k/l` |
| Open a new horizontal / vertical Nvim terminal | `Space h` / `Space v` |
| Toggle a reusable horizontal / vertical / floating terminal | `Alt-h` / `Alt-v` / `Alt-i` |
| Leave terminal-input mode / find hidden terminals | `Ctrl-x` / `Space pt` |
| Enter tmux copy mode, select, copy, return to typing | `prefix v`, then `v`, `y`; `i` cancels back to input |
| Open Git or files in a terminal UI | `lg` / `y` (full profile) |
| See practical examples for a command | `tldr <command>`; use `tldr --update` if its page cache is empty |
| Load project-local environment variables | create `.envrc`, then run `direnv allow` |
| Common aliases | `ll`, `la`, `tree`, `gs`, `gd`, `gl`, `gp`, `ga`, `gc`, `gco`, `mkcd` |

`prefix` means `Ctrl-a`, not tmux's default `Ctrl-b`.
...
hand you the build box's layout on the GPU box.

**Copy lands on your laptop.** Yank in nvim, or press `prefix v` in tmux for a
visible COPY mode, select with `v`, and copy with `y`; the text goes to the
clipboard of the machine you are sitting at, carried by the SSH connection you
already have — no X11 forwarding, no root, no daemon. Your local terminal must
allow OSC 52 clipboard writes. VS Code and Cursor ship this disabled; set
`terminal.integrated.enableClipboardWrite` to `true` locally.
...
you list your own in `~/.config/envup/project-dirs`.
...
The `nvim` module symlinks the NvChad config to `~/.config/nvim`, restores the
pinned plugins, and installs and verifies its language tools. NvChad needs
**nvim >= 0.10**, and the distro package is often older —
...
package sources. On Linux hosts with glibc older than 2.34 it selects the
compatible Neovim 0.10.4 release; newer official builds cannot start there.
...
- `skip` — explicitly skip provisioning plugins, LSP servers and formatters.

`./envup clean nvim` clears plugin/cache state if it gets stuck; the next
install restores from the lock.

**Language support is ready after install.** `envup install nvim` waits for the
tools below and runs each binary before reporting the module healthy. A failed
or interrupted download leaves nvim usable but marks the module degraded; rerun
the same command to finish it.

| Files | Language server | Formatter |
|---|---|---|
| C / C++ | clangd (index, completion, diagnostics, clang-tidy) | clang-format |
| Python | pyright | Ruff |
| Lua | lua-language-server | StyLua |
| shell | bash-language-server | shfmt |
| JSON / YAML / Markdown | — | Prettier |

The main code-navigation keys are available whenever an LSP attaches (the
NvChad leader key is `Space`):

| Action | Key |
|---|---|
| Definition / declaration | `gd` / `gD` |
| References / implementation / type definition | `gr` / `gi` / `gy` |
| Hover / signature help | `K` / `<leader>lh` |
| Rename / code action | `<leader>rn` / `<leader>ca` |
| Document / workspace symbols | `<leader>ls` / `<leader>lS` |
| Previous / next diagnostic; full message | `[d` / `]d`; `<leader>df` |

Hover and signature help use a rounded, high-contrast floating window. Press
`K` once to open hover and again to focus it; inside it, `K` or `<C-w>p`
returns to the source window and `Esc` closes it. Empty
navigation results are no longer silent either: Nvim names the missing result
and suggests what to check. `gi` is mainly for interface or virtual-method
implementations, while `gy` follows a symbol's **type definition**; use `gd` for
ordinary functions/variables and `gD` when you specifically want a declaration.

Accurate C/C++ navigation depends on the project's `compile_commands.json`.
clangd is allowed to query trusted GCC/Clang/HIP compilers installed on the
current machine for their system headers. When CMake runs in Docker but Nvim
runs on the host, use `envup clangd sync` in the project. It discovers arbitrary
bind-mount paths through `docker inspect`, writes a host-view database to the
Nvim cache, and leaves the Docker database untouched. It normally identifies
the task container from the exact project mount and database creation time;
only a real ambiguity needs `--container NAME`. Run `:LspRestart` after syncing.
Nvim warns again if Docker later regenerates the source database.
...
- Format on save runs **only where the project ships its own style config**
  ...
  Use `:ConformInfo` to inspect the formatter selected for the current buffer.
...
| `ENVUP_BACKUP_DIR` | `~/.dotfiles_backup/<timestamp>` | Where any pre-existing file, directory or foreign symlink at a link target is moved before the managed symlink is created. One directory per run; the path inside it mirrors the original. |
| `ENVUP_STATE_DIR` | `~/.local/state/envup` | Manifest, logs and adopt's stash. On a shared HOME this becomes `.../envup/hosts/<hostname>` so machines cannot overwrite one another's state; an explicitly set value is left alone. Honours `XDG_STATE_HOME` when the historical default is not already in use. |
...
| `ENVUP_PROJECT_DIRS` | common project roots | Colon-separated directory globs searched by `ts` and `prefix f`. |
| `ENVUP_TMUX_SESSION` | `main` | Session name `tm` creates when there is nothing to restore. |
| `ENVUP_TMUX_RESTORE_WAIT` | `8` | Seconds `tm` waits for a saved layout to reappear. |
| `ENVUP_TS_POPUP` | detected | Set to `1` or `0` to force the tmux project picker to use or avoid a popup. |
...
If a link target is an existing file, directory or foreign symlink (e.g. a
`~/.zshrc` you wrote by hand), envup **always backs it up** to
`~/.dotfiles_backup/<timestamp>/` before
...
# Local HOME; a shared HOME adds hosts/<hostname>/:
ls ~/.local/state/envup/logs/
ls ~/.local/state/envup/hosts/"$(hostname -s)"/logs/
```
...
**`setlocale: cannot change locale`** — envup does not force a locale: it picks the first of `en_US.UTF-8` / `C.UTF-8` that the machine actually has, sets only `LANG`, and never sets `LC_ALL`. If you still see the warning, something else in your environment is setting it — `envup doctor` reports both cases.
...
with `git stash`. HEAD detached, which is what an earlier `upgrade --ref v0.1.0`
