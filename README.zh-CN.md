...
# 安装 standard profile（shell、tmux、搜索工具、jq 和 tldr）
./envup install
...
./envup clangd sync [--container NAME] [PATH]                  # 把 Docker 编译数据库转换给 Host Nvim
./envup clangd status [PATH]                                   # 查看项目正在使用的 clangd 数据库
./envup clangd clean [PATH]                                    # 删除项目的转换缓存
...
| `standard`（默认） | `+ tmux fzf ripgrep fd bat eza zoxide atuin delta direnv jq tealdeer` | 典型开发机 |
| `full` | `+ nvim lazygit yazi` | 高级工作站 |
...
# profiles/full.sh = standard + 编辑器和终端 TUI
...
| `jq` | 处理 API、日志和命令管道中的 JSON | — |
| `tealdeer` | 通过 `tldr` 快速查看命令实例 | — |
| `nvim` | Neovim + NvChad（插件版本由 lazy-lock.json 钉住）| — |
| `lazygit` | 用 TUI 完成暂存、日志、分支和 rebase | `git` |
| `yazi` | Vim 风格终端文件管理器；`y` 退出后保留所在目录 | — |

安装 full profile 后，用 `lg` 启动 lazygit、用 `y` 启动 Yazi；存在 `fzf`
时普通 Tab 补全会自动变成模糊选择。默认 profile 已提供 `tldr <命令>` 和 `jq`。

### 日常使用速查

首次安装后用 `exec zsh` 进入新环境。拉取了新增模块的版本后，要带上 profile，
这样新模块也会被安装：

```bash
./envup upgrade --profile standard   # 或 full
exec zsh
```

| 想做什么 | 命令 / 按键 |
|----------|-------------|
| 在 zsh 输入模式和普通模式间切换 | `Esc` 进入普通模式，`i` / `a` 回到输入模式 |
| 模糊补全命令、参数或路径 | `Tab`（安装 fzf 后由 `fzf-tab` 提供）|
| 搜索历史命令 | `Ctrl-R`（优先 Atuin，fzf 作为回退）|
| 选择文件插入命令行 / 选择目录并进入 | `Ctrl-T` / `Alt-C` |
| 跳到访问过的目录 / 交互选择目录 | `z <名称>` / `zi` |
| 回到恢复后的 tmux 工作区 / 选择项目 session | `tm` / `ts` |
| 跨 tmux pane 和 nvim split 移动 | `Ctrl-h/j/k/l` |
| 在 Nvim 新建水平 / 垂直终端 | `Space h` / `Space v` |
| 切换可复用的水平 / 垂直 / 浮动终端 | `Alt-h` / `Alt-v` / `Alt-i` |
| 从 Nvim 终端输入回普通模式 / 找回隐藏终端 | `Ctrl-x` / `Space pt` |
| 进入 tmux 复制模式、选择、复制、回到输入 | `prefix v`，然后 `v`、`y`；按 `i` 取消并回到输入 |
| 用终端 UI 操作 Git / 文件 | `lg` / `y`（full profile）|
| 快速看某条命令的实用示例 | `tldr <命令>`；页面缓存为空时运行 `tldr --update` |
| 加载项目目录自己的环境变量 | 创建 `.envrc`，然后运行 `direnv allow` |
| 常用别名 | `ll`、`la`、`tree`、`gs`、`gd`、`gl`、`gp`、`ga`、`gc`、`gco`、`mkcd` |

这里的 `prefix` 是 `Ctrl-a`，不是 tmux 默认的 `Ctrl-b`。

### 在远端机器上干活

tmux 模块里有三件事只在 SSH 场景下才有意义：

**重启不会让你丢掉现场。** 你正在用的那套布局 —— 窗口、pane、各自的目录、回滚缓冲、nvim 里开着的文件 —— 每 5 分钟存一次；重新登录后敲一个 `tm`（全名 `tmux-resume`）就回到那个状态。**这个命令存在是因为直接敲 `tmux` 是错的**：拉起 server 才会触发恢复，但恢复是异步的（要先 sleep 1 秒等插件加载完），而你的 `tmux` 已经立刻建好了一个空 session —— 你会停在一个空壳里，真正的布局一秒后出现在旁边，屏幕上没有任何提示。`tm` 会拉起 server、等恢复落地、再 attach 到回来的那些 session 上。**登录时不会自动帮你 attach**：自动路径必须替你猜"这条连接要不要 multiplexer"，猜错的结果就是把你丢进一个不是你原来那个的 session。存档按机器分开，所以 NFS 共用 home 时不会在 GPU 机器上给你恢复出编译机的布局。

**复制的东西落到你自己的电脑上。** 在 nvim 里 yank、或在 tmux 里按 `prefix v` 进入带状态栏提示的 COPY 模式、`v` 选择、`y` 复制，文字会经由你已经建好的那条 SSH 连接，进到你面前这台机器的剪贴板 —— 不需要 X11 转发、不需要 root、不需要额外的守护进程。本地终端必须允许 OSC 52 写剪贴板；VS Code 和 Cursor 默认关闭，需要在本地把 `terminal.integrated.enableClipboardWrite` 设为 `true`。

**`prefix f` 切项目。** fzf 列出你的项目目录，选中后连上（或新建）一个以项目命名的 tmux session —— 所以选一个已经开着的项目是回到它，而不是再开一份。tmux ≥ 3.2 会在当前 pane 上开一个 popup，更老的版本退回临时窗口 —— 按键时自己问，不用配。shell 里也可以用 `ts`。默认扫 `~/work/*`、`~/src/*`、`~/projects/*`、`~/dev/*`、`~/repos/*`、`~/go/src/*/*`，要改就在 `~/.config/envup/project-dirs` 里一行一个写自己的。
...
`nvim` 模块把 NvChad 配置链到 `~/.config/nvim`，还原钉住的插件，并安装、验证语言
工具。NvChad 需要
**nvim >= 0.10**，而发行版自带的往往更老 —— Debian stable 和 RHEL 都是。envup 自己
处理：引擎发现版本不够，就继续往下走 provider 链，改装官方 release 到
`~/.local/bin`。envup 从不动你的系统软件源。Linux glibc 低于 2.34 时会自动选择
兼容的 Neovim 0.10.4；更新的官方构建在这些服务器上无法启动。
...
- `sync` —— 在版本约束内更新到最新**并重写锁文件**；之后把新的 `lazy-lock.json`
  提交上去，就能推给所有机器。
- `skip` —— 明确跳过插件、LSP server 和 formatter 的部署。

`./envup clean nvim` 清掉插件/缓存状态，下次安装会从锁文件还原。

**装完即可使用语言功能。** `envup install nvim` 会等待下列工具安装完成，并实际运行
每个二进制后才把模块标为健康。下载失败或中断时 nvim 本身仍可用，但模块会显示为
degraded；重新执行同一条安装命令即可续完。

| 文件 | Language server | Formatter |
|---|---|---|
| C / C++ | clangd（索引、补全、诊断、clang-tidy） | clang-format |
| Python | pyright | Ruff |
| Lua | lua-language-server | StyLua |
| shell | bash-language-server | shfmt |
| JSON / YAML / Markdown | — | Prettier |

LSP attach 后可直接使用下面这些代码导航键（NvChad 的 leader 是 `Space`）：

| 功能 | 按键 |
|---|---|
| 跳到定义 / 声明 | `gd` / `gD` |
| 查找引用 / 实现 / 类型定义 | `gr` / `gi` / `gy` |
| 悬浮说明 / 函数签名 | `K` / `<leader>lh` |
| 重命名 / Code Action | `<leader>rn` / `<leader>ca` |
| 当前文件 / 工作区符号 | `<leader>ls` / `<leader>lS` |
| 上一个 / 下一个诊断；完整信息 | `[d` / `]d`；`<leader>df` |

`K` 和函数签名使用带圆角边框、与正文有明显色差的浮窗。第一次按 `K` 打开说明，
再按一次进入浮窗；浮窗内按 `K` 或 `<C-w>p` 回到源码窗口，按 `Esc` 关闭。导航请求没有结果时也不再
静默：Nvim 会说明没有找到哪一类目标，并提示下一步。注意 `gi` 主要查接口或虚函数的
实现，`gy` 查符号的**类型定义**；普通函数/变量优先用 `gd`，只有声明时用 `gD`。

C/C++ 跳转的准确度依赖项目自己的 `compile_commands.json`。clangd 会查询当前机器上
可信的 GCC/Clang/HIP 编译器以获得系统头文件。如果 CMake 在 Docker 中运行、Nvim 在
Host 运行，直接使用数据库会因为绝对路径属于不同文件系统而失败。在项目目录执行：

```bash
envup clangd sync
```

命令会从 CMake cache 找到容器源码根目录，从 `docker inspect` 自动读取任意
`Host Source → Container Destination` mount（不假设 `/workspace` 或任何 Host 路径），
并在 Nvim cache 中生成 Host 视角的数据库；原文件不会被修改。通常会根据项目 mount 和
数据库/任务容器的创建时间自动找到生成者。多个环境仍无法区分时才需要
`--container NAME`。同步完成或 Docker 重新 configure 后执行 `:LspRestart`；若原数据库
之后发生变化，Nvim 和 `envup clangd status` 都会提示重新同步。
...
- 保存时格式化**只在项目自带风格配置时**才跑（`.clang-format`、`stylua.toml`、
  ...
  `vim.g.envup_format_always = true` 让它无条件跑。用 `:ConformInfo` 查看当前 buffer 选中的
  formatter。
...
| `ENVUP_BACKUP_DIR` | `~/.dotfiles_backup/<时间戳>` | 链接目标处已有文件、目录或其它软链时，先搬到这里再建链。每轮一个目录，里面保留原来的相对路径，既不撞名也看得出原来在哪。 |
| `ENVUP_STATE_DIR` | `~/.local/state/envup` | manifest、日志、adopt 暂存；共享 HOME 上自动变为 `.../envup/hosts/<hostname>`，避免机器间覆盖状态。显式设置此变量时不再自动分层。认 `XDG_STATE_HOME`，但只在尚未使用历史默认路径时才认。 |
...
| `ENVUP_PROJECT_DIRS` | 常见项目目录 | `ts` 与 `prefix f` 搜索的目录 glob，以冒号分隔。 |
| `ENVUP_TMUX_SESSION` | `main` | 没有可恢复布局时，`tm` 创建的 session 名称。 |
| `ENVUP_TMUX_RESTORE_WAIT` | `8` | `tm` 等待已保存布局恢复的秒数。 |
| `ENVUP_TS_POPUP` | 自动探测 | 设为 `1` 或 `0`，强制 tmux 项目选择器使用或不使用 popup。 |
...
如果软链目标处已经有文件、目录或其它软链（比如你自己手写的 `~/.zshrc`），envup **一定会
...
- **备份而非覆盖**：`safe_link` 会先把目标处已有的文件、目录或其它软链移到 `~/.dotfiles_backup/<时间戳>/`，并且**保留原来的相对路径**（`~/.config/git/ignore` 存成 `<备份>/.config/git/ignore`）。这样既能从备份本身看出原来在哪，两个同名路径也不会互相覆盖。
...
- **有日志**：任何会改动机器的命令都会在 `$ENVUP_STATE_DIR/logs/` 留一份带时间戳的日志，`doctor`（尤其是 `--fix`）和 `adopt` 也在内。普通 HOME 默认是 `~/.local/state/envup`；检测到共享 HOME 时按 hostname 放到 `~/.local/state/envup/hosts/<host>`，旧的未分层 manifest 会在首次写操作时复制迁移且保留原件。
...
# 普通 HOME；共享 HOME 则多一层 hosts/<hostname>/：
...
**`setlocale: cannot change locale`** —— envup 不会硬设 locale：它从
...
（例如 `upgrade --ref v0.1.0` 留下的）→ 在联网之前就拦下，并给出
